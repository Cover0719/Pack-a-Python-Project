...
- 主要参数：
  ...
  - `add_file`：资源文件列表，多个文件时请按多行写入，每行仅写一个文件，顺序会保留；路径使用项目内相对路径，如 `/assets/package.ico`
  - `ico_path`：图标文件路径，使用项目内相对路径
  - `setting`：勾选设置，格式为 `生成单文件,加快打包,隐藏命令行,减少包体积`，例如 `1,1,1,0`

- 额外说明：
  - `add_file` 可以重复出现多次；每次只写一个文件路径，程序会按出现顺序追加到资源列表中
  - `setting` 中的 4 个值分别对应：生成单文件 / 加快打包 / 隐藏命令行 / 减少包体积
